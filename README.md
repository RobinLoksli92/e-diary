...
Для использования возможностей скрипта нужно открыть командную строку и написать `python manage.py shell` и импортировать его функции: `from scripts import fix_marks, remove_chastisements, create_commendation`.

1. Исправление оценок:

После запуска напишите `fix_marks` и передайте в скобках имя ученика, например:
```
...
После запуска напишите `remove_chastisements` и передайте в скобках имя ученика, например:
```
...
После запуска напишите `create_commendation` и передайте в скобках имя ученика и название предмета, например:
```
create_commendation('Иванов Иван', Биология)
```
